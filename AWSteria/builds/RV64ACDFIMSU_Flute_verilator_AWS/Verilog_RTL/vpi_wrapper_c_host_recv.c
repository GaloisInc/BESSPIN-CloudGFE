 ...
 *     #include "vpi_wrapper_c_host_recv.h"
 *     void (*vlog_startup_routines[])() = { c_host_recv_vpi_register, 0u };
 * 
 * For a Verilog simulator which requires a .tab file, use the following entry:
 * $imported_c_host_recv call=c_host_recv_calltf size=632 acc=rw:%TASK
 * 
 * For a Verilog simulator which requires a .sft file, use the following entry:
 * $imported_c_host_recv vpiSysFuncSized 632 unsigned
 */
#include <stdlib.h>
#include <vpi_user.h>
#include "bdpi.h"
...
void c_host_recv(unsigned int *, char );

/* VPI wrapper function */
PLI_INT32 c_host_recv_calltf(PLI_BYTE8 *user_data)
{
  vpiHandle hCall;
  char arg_1;
...
/* sft: $imported_c_host_recv vpiSysFuncSized 632 unsigned */

/* tab: $imported_c_host_recv call=c_host_recv_calltf size=632 acc=rw:%TASK */

PLI_INT32 c_host_recv_sizetf(PLI_BYTE8 *user_data)
{
  return 632u;
}

/* VPI wrapper registration function */
void c_host_recv_vpi_register()
{
  ...
  tf_data.tfname = "$imported_c_host_recv";
  tf_data.calltf = c_host_recv_calltf;
  tf_data.compiletf = 0u;
  tf_data.sizetf = c_host_recv_sizetf;
  tf_data.user_data = "$imported_c_host_recv";
